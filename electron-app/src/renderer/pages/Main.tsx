...
export default function Main() {
    ...
    const [SPSPath, setSPSPath] = useState<string | null>(null);
    const [SPSProxyPath, setSPSProxyPath] = useState<string | null>(null);
    ...
    const handleExportButton = async () => {
        ...
        const fileExists =
            await window.electron.ipcRenderer.checkFileExists(outputPath);
        if (fileExists) {
            setModalOpen(true);
            return;
        }
    ...
    const handleMerge = async () => {
        ...
        const originalCode =
            await window.electron.ipcRenderer.readFile(outputPath);
            ...
            <main className="flex-1 px-6 py-8">
                ...
                {mergerOpen && originalCode && modifiedCode ? (
                    ...
                    <form className="max-w-2xl mx-auto flex flex-col gap-4 bg-gray-300 p-8 rounded-lg shadow-md">
                        ...
                        {/* SPS Proxy Path Selection */}
                        <Dropzone
                            id="sps-proxy-dropzone"
                            label="Select SPS Proxy file"
                            accept=".cs"
                            onChange={(file) => {
                                if (file)
                                    setSPSProxyPath(
                        ...
                        <button
                            ...
                            disabled={
                                !amlPath || !outputPath || exportButtonLoading
                            }
                        >
